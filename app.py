...
TOOLS_LIST = ['serpapi', 'wolfram-alpha', 'pal-math', 'pal-colored-objects', 'news-api', 'tmdb-api',
              'open-meteo-api','davinci']  # 'google-search'
...
LOOPING_TALKING_HEAD = "videos/elon_musk.mp4"
...
def transcribe(aud_inp, whisper_lang):
    ...
    if whisper_lang != WHISPER_DETECT_LANG:
        ...
        options = whisper.DecodingOptions(language=whisper_lang_code,fp16=False)
...
def load_chain(tools_list, llm):
    ...
    if llm:
        ...
        tools = load_tools(tool_names, llm=llm, news_api_key=news_api_key, tmdb_bearer_token=tmdb_bearer_token,wolfram_alpha=wolfram_alpha_appid)
...
def set_openai_api_key(api_key):
    ...
    if api_key and api_key.startswith("sk-") and len(api_key) > 50:
        ...
        llm = OpenAI(model_name="text-davinci-003",temperature=0, max_tokens=MAX_TOKENS)
...
def create_html_video(file_name, width):
    ...
    html_video = f'<video width={width} height={width} autoplay muted loop><source src={temp_file_url} type="video/mp4" poster="elon_musk.png"></video>'
    return html_video
...
def do_html_video_speak(words_to_speak, azure_language):
    ...
    headers = {"Authorization": f"Bearer {os.environ.get('EXHUMAN_API_KEY')}"}
    body = {
        'bot_name': 'Elon Musk',
    ...
    if isinstance(res.content, bytes):
        ...
        html_video = f'<video width={TALKING_HEAD_WIDTH} height={TALKING_HEAD_WIDTH} autoplay><source src={temp_file_url} type="video/mp4" poster="elon_musk.png"></video>'
    else:
...
with gr.Blocks(css=".gradio-container {background-color: lightgray}") as block:
    ...
    with gr.Tab("Settings"):
        ...
        openai_api_key_textbox = gr.Textbox(label="Enter your API key:",placeholder="Paste your OpenAI API key (sk-...)",
                                                show_label=True, lines=1, type='password')
